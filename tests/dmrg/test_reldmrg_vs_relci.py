...
These define the target API for `forte2.dmrg.RelDMRGSolver`, which must be a
drop-in replacement for `forte2.ci.RelCISolver` under the `CI` driver. The
...
exact RelCI solver, computed in-test on the same spinors/active space.
"""

import pytest

from forte2 import System, RHF, GHF, SpinorUpcaster, CI, RelCISolver
...
def test_reldmrg_vs_relci_h2(tmp_path):
    ...
    dmrg = CI(
        RelDMRGSolver(nel=2, active_orbitals=4, dmrg_params=TIGHT(str(tmp_path)))
    )(conv)
    dmrg.run()

    assert dmrg.ci_solver.two_component
...
def test_reldmrg_vs_relci_hf_x2c(tmp_path):
    ...
    ci = CI(RelCISolver(nel=10, core_orbitals=2, active_orbitals=8))(scf)
    ci.run()
