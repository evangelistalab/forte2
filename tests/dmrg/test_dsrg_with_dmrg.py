...
DSRG-MRPT2 reaches the active-space solver through the generic RDM API: it
calls ``ci_solver.make_average_cumulant(order)`` (see
``DSRG_MRPT2.get_integrals``), which in turn calls ``make_average_rdm`` on
``CIBase``. This exercises DMRGSolver's 3-RDM machinery through that generic
path (as opposed to the direct ``make_rdm`` calls in test_dmrg_rdms.py), plus
the ``set_ints`` / ``reset_eigensolver`` / rerun contract used for reference
relaxation. The DMRG-CASSCF + DSRG-MRPT2 energy must match the exact
CI-CASSCF + DSRG-MRPT2 energy computed in-test.
"""
