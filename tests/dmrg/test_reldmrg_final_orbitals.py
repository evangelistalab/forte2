...
def _run_reldmrg_and_relci(final_orbitals, tmp_path):
    ...
    relci = CI(
        ...
        final_orbitals=(
            final_orbitals if final_orbitals == "semicanonical" else "original"
        ),
    )(scf)
    relci.run()
...
def test_reldmrg_final_orbitals_semicanonical(tmp_path):
    ...
    assert reldmrg.E_ci[0] == approx(relci.E_ci[0].real)
...
def test_reldmrg_final_orbitals_natural(tmp_path):
    ...
    dmrg_occs = np.sort(np.linalg.eigvalsh(reldmrg.make_average_rdm(1)))[::-1]
    relci_occs = np.sort(np.linalg.eigvalsh(relci.make_average_rdm(1)))[::-1]
