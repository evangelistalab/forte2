...
(general-spin, complex) block2 path used by RelDMRGSolver, exercised through
the ``CI`` driver's ``do_transition_dipole``.
"""

import pytest

from forte2 import System, GHF, CI, RelCISolver, X2CParams
...
def test_reldmrg_transition_dipoles_vs_relci(tmp_path):
    ...
    assert reldmrg.ci_solver.vertical_transition_energies[(0, 3)] == approx(
        relci.ci_solver.vertical_transition_energies[(0, 3)].real
    )
    assert reldmrg.ci_solver.oscillator_strengths[(0, 3)] == approx(
        relci.ci_solver.oscillator_strengths[(0, 3)].real
    )
