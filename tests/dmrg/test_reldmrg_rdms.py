...
def test_reldmrg_1rdm_hermitian_and_trace(tmp_path):
    ...
    g1 = dmrg.make_rdm(0, order=1, spin_type="so")
...
def test_reldmrg_rdm_energy_roundtrip(tmp_path):
    ...
    g1 = dmrg.make_rdm(0, order=1, spin_type="so")
    g2 = dmrg.make_rdm(0, order=2, spin_type="so")
    ...
    assert e.real == approx(dmrg.E_ci[0])
...
def test_reldmrg_rdms_match_relci(tmp_path):
    ...
    scf = GHF(charge=0)(system)
    ci = CI(RelCISolver(nel=NEL, core_orbitals=NCORE, active_orbitals=NACT_SPINORS))(
        scf
    )
    ci.run()

    # RDMs are limited by DMRG truncation error, looser than the energy.
    g1_dmrg = dmrg.make_rdm(0, order=1, spin_type="so")
    g1_ci = ci.make_rdm(0, order=1, spin_type="so")
    assert np.linalg.norm(g1_dmrg - g1_ci) < 1e-5

    g2_dmrg = dmrg.make_rdm(0, order=2, spin_type="so")
    g2_ci = ci.make_rdm(0, order=2, spin_type="so")
...
def test_reldmrg_3rdm_matches_relci(tmp_path):
    ...
    g3_dmrg = dmrg.make_rdm(0, order=3, spin_type="so")
    g3_ci = ci.make_rdm(0, order=3, spin_type="so")
...
def test_reldmrg_3rdm_partial_trace(tmp_path):
    ...
    g2 = dmrg.make_rdm(0, order=2, spin_type="so")
    g3 = dmrg.make_rdm(0, order=3, spin_type="so")
...
def test_reldmrg_transition_rdms_hermiticity(tmp_path):
    ...
    t1_01 = dmrg.make_rdm(0, 1, order=1, spin_type="so")
    t1_10 = dmrg.make_rdm(1, 0, order=1, spin_type="so")
    assert np.linalg.norm(t1_10 - t1_01.conj().transpose(1, 0)) < 1e-6

    t2_01 = dmrg.make_rdm(0, 1, order=2, spin_type="so")
    t2_10 = dmrg.make_rdm(1, 0, order=2, spin_type="so")
    assert np.linalg.norm(t2_10 - t2_01.conj().transpose(2, 3, 0, 1)) < 1e-6

    t3_01 = dmrg.make_rdm(0, 1, order=3, spin_type="so")
    t3_10 = dmrg.make_rdm(1, 0, order=3, spin_type="so")
    assert np.linalg.norm(t3_10 - t3_01.conj().transpose(3, 4, 5, 0, 1, 2)) < 1e-6
