...
def test_dmrg_1rdm_trace(tmp_path):
    ...
    g1 = dmrg.make_rdm(0, order=1, spin_type="sf")
    assert g1.shape == (6, 6)
...
def test_dmrg_rdm_energy_roundtrip(tmp_path):
    ...
    g1 = dmrg.make_rdm(0, order=1, spin_type="sf")
    g2 = dmrg.make_rdm(0, order=2, spin_type="sf")
    ints = dmrg.ci_solver.sub_solvers[0].ints

    e = ints.E
    e += np.einsum("pq,pq->", ints.H, g1)
    e += 0.5 * np.einsum("pqrs,pqrs->", ints.V, g2)

    assert e == approx(dmrg.E_ci[0])
...
def test_dmrg_rdms_match_fci(tmp_path):
    ...
    g1_dmrg = dmrg.make_rdm(0, order=1, spin_type="sf")
    g1_ci = ci.make_rdm(0, order=1, spin_type="sf")
    assert np.linalg.norm(g1_dmrg - g1_ci) < 1e-5

    g2_dmrg = dmrg.make_rdm(0, order=2, spin_type="sf")
    g2_ci = ci.make_rdm(0, order=2, spin_type="sf")
...
def test_dmrg_average_rdms_roundtrip(tmp_path):
    ...
    g1 = dmrg.make_average_rdm(1)
    g2 = dmrg.make_average_rdm(2)
    ints = dmrg.ci_solver.sub_solvers[0].ints
    ...
    assert e == approx(dmrg.ci_solver.compute_average_energy())
...
def test_dmrg_3rdm_matches_fci(tmp_path):
    ...
    g3_dmrg = dmrg.make_rdm(0, order=3, spin_type="sf")
    g3_ci = ci.make_rdm(0, order=3, spin_type="sf")
...
def test_dmrg_3rdm_partial_trace(tmp_path):
    ...
    g2 = dmrg.make_rdm(0, order=2, spin_type="sf")
    g3 = dmrg.make_rdm(0, order=3, spin_type="sf")
    # CAS(6,6): 6 active electrons.
...
def test_dmrg_transition_rdms_match_fci(tmp_path):
    ...
    dmrg = _build_dmrg_h2o(
        ...
        str(tmp_path),
        nroots=3,
        dmrg_params=CONVERGED_DMRG_PARAMS(str(tmp_path)),
    )
    ci = _build_ci_h2o(system, nroots=3)

    # sanity: the roots are the ones we expect (and non-degenerate)
    for r in range(3):
        assert dmrg.E_ci[r] == approx(ci.E_ci[r])
    assert abs(ci.E_ci[1] - ci.E_ci[0]) > 1e-2
    assert abs(ci.E_ci[2] - ci.E_ci[1]) > 1e-2

    for left, right in [(0, 1), (0, 2), (1, 2)]:
        for order in (1, 2, 3):
            t_dmrg = dmrg.make_rdm(left, right, order=order, spin_type="sf")
            t_ci = ci.make_rdm(left, right, order=order, spin_type="sf")
            assert _match_up_to_sign(t_dmrg, t_ci) < 1e-5
...
def test_dmrg_diagonal_rdm_via_transition_api(tmp_path):
    ...
    for order in (1, 2, 3):
        diag = dmrg.make_rdm(1, order=order, spin_type="sf")
        diag_explicit = dmrg.make_rdm(1, 1, order=order, spin_type="sf")
