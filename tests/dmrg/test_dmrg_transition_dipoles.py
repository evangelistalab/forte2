...
The transition-dipole machinery in ``CIBase.compute_transition_properties``
is representation-agnostic -- it only calls ``make_rdm(left_root, right_root,
order=1, ...)`` for cross-root (transition) 1-RDMs, which ``DMRGSolver`` and
``RelDMRGSolver`` provide via block2's ``get_npdm(..., bra=...)``. This test
exercises that binding end-to-end (through the ``CI`` driver's
``do_transition_dipole``) and checks the resulting oscillator strengths and
vertical transition energies against the exact CI reference (computed
in-test), reusing the same system as test_ci_tdm in
...
def test_dmrg_transition_dipoles_vs_fci(tmp_path):
    ...
    assert dmrg.ci_solver.vertical_transition_energies[(0, 6)] == approx(
        ci.ci_solver.vertical_transition_energies[(0, 6)]
    )
    assert dmrg.ci_solver.oscillator_strengths[(0, 6)] == pytest.approx(
        ci.ci_solver.oscillator_strengths[(0, 6)], abs=1e-4
    )
