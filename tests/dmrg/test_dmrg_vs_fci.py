...
These tests define the target API for `forte2.dmrg.DMRGSolver`, which must be a
drop-in replacement for `forte2.ci.CISolver` under the `CI` driver. With a large
enough bond dimension, DMRG on a small active space must reproduce the FCI energy
from the exact CI solver, computed in-test on the same orbitals/active space.
"""
...
def test_dmrg_vs_fci_h2(tmp_path):
    ...
    ci = CI(CISolver(states=state, active_orbitals=[0, 1]))(rhf)
    ci.run()
