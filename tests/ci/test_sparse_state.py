...
def test_sparse_vector_complex():
    ...
    # different lengths
    psi3 = forte2.SparseState({det("2"): 2.0 + 1j, det("+-"): 1.0 - 1j})
...
def test_sparse_vector_addition():
    psi1 = forte2.SparseState({det("2"): 2.0 + 1j})
    psi2 = forte2.SparseState({det("2"): 1.0 - 0.5j, det("+-"): 0.5 + 0.5j})
    psi3 = psi1 + psi2
    assert psi3[det("2")] == pytest.approx(3.0 + 0.5j, abs=1e-9)
    assert psi3[det("+-")] == pytest.approx(0.5 + 0.5j, abs=1e-9)

    psi1 += psi2
    assert psi1[det("2")] == pytest.approx(3.0 + 0.5j, abs=1e-9)
    assert psi1[det("+-")] == pytest.approx(0.5 + 0.5j, abs=1e-9)
...
def test_sparse_vector_scalar_multiplication():
    psi1 = forte2.SparseState({det("2"): 2.0 + 1j, det("+-"): 1.0 + 2.0j})
    scalar = 2.0 + 3.0j

    # this calls the __rmul__ of SparseState
    psi2 = scalar * psi1
    assert psi2[det("2")] == pytest.approx((1.0 + 8.0j), abs=1e-9)
    assert psi2[det("+-")] == pytest.approx((-4.0 + 7.0j), abs=1e-9)
    ...
    assert psi1[det("+-")] == pytest.approx((-4.0 + 7.0j), abs=1e-9)


def test_sparse_vector_norm():
    import math

    psi = forte2.SparseState({det("2"): 2.0 + 1j, det("+-"): 1.0 - 3.0j})
