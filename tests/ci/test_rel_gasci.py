...
from forte2 import CI, RHF, ROHF, RelCISolver, RelState, System
...
def test_rel_gasci_rhf_1():
    ...
    conv = SpinorUpcaster(apply_random_phase=True)(rhf)
    ci = CI(
        RelCISolver(
...
def test_rel_gasci_rhf_5():
    ...
    assert ci.E_ci[3] == approx(-55.819535370117)
...
def test_rel_gasci_rohf_3():
    ...
    ci = CI(
        RelCISolver(
            active_orbitals=(2, 12), states=RelState(nel=10, gas_min=[0], gas_max=[1])
        )
    )(conv)
    ci.run()
