...
2c SS-CASSCF/GASSCF & spinor-upcast nonrelativistic, SF-X2C-1e, or SO-X2C-1e & point \\
\bottomrule
\end{tabular}
\end{center}
...
For the supported SCF gradients, Gaussian nuclei change three pieces
consistently: $V^\xcoord$, $W^\xcoord$, and $E_\mathrm{NN}^\xcoord$.  The last is
...
removed exactly as in the energy.  CASSCF/GASSCF gradients currently require
point nuclei, as summarized above.
...
frozen, or frozen inter-GAS rotations.  Both the orbital optimization and CI
roots must be converged before a gradient is evaluated.  The excluded cases
require response terms outside the stationary single-state Lagrangian used
here.
