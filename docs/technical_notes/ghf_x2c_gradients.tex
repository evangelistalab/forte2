...
one-electron exact-two-component (X2C-1e) Hamiltonian derivatives in Forte2.
...
SS-CASSCF/GASSCF & nonrelativistic or spin-free X2C-1e & point \\
2c SS-CASSCF/GASSCF & nonrelativistic, SF-X2C-1e, or SO-X2C-1e & point \\
\bottomrule
\end{tabular}
\end{center}
...
The term ``analytic gradient'' below means that the SCF energy derivative and
the X2C matrix response are evaluated analytically.  The same-basis $W$
integrals used by X2C are analytic libcint derivatives.  A fourth-order finite
difference remains only as a public cross-basis fallback for
\texttt{opVop\_deriv\_matrices}; the X2C driver does not use that fallback.
...
\end{align}
The public \texttt{opVop} and \texttt{opVop\_deriv\_matrices} order is
\begin{equation}
 [W_0,W_x,W_y,W_z]
 =[hat p\cdot V\hat p,
...
\section{Analytic derivative of the X2C Hamiltonian}

\subsection{Derivative integral matrices}

Every nuclear coordinate requires
\begin{equation}
 S^\xcoord,\quad T^\xcoord,\quad V^\xcoord,\quad
 [W_0^\xcoord,W_x^\xcoord,W_y^\xcoord,W_z^\xcoord].
 \label{eq:x2c_required_derivatives}
\end{equation}
Libint2 supplies matrix-valued first derivatives of $S$, $T$, and point-nucleus
$V$.  Gaussian-nucleus $V^\xcoord$ and all four $W^\xcoord$ components use
libcint.  The relevant libcint kernels are
...
\end{align}
The libcint component and transpose conventions are normalized once inside
\texttt{opVop\_deriv}\allowbreak\texttt{\_matrices}; downstream X2C code sees only the public order
in \cref{eq:opvop_order}.

\subsection{Orthogonalizer response}

Inside the retained overlap space Forte2 uses a parallel-transport gauge,
\begin{equation}
 O^\xcoord_\parallel
...
one-electron response, not an iterative CPHF equation.
...
\cref{eq:contracted_hx2c} completes $h_\mathrm{X2C}^\xcoord$.
...
components, applies the same elementwise SNSO map to the reference and
derivative spin-dependent components, reassembles the spinor matrix, and only
then applies $P$ and $P^\xcoord$.  There is therefore no separate derivative of
the SNSO factors.

\subsection{Density-contracted adjoint used in production}

The matrix-valued forward response described above is retained as
\texttt{hcore\_deriv} for verification and for callers that require
$h_\mathrm{X2C}^\xcoord$ itself.  Energy gradients require only the scalar
contraction in \cref{eq:x2c_hcore_contraction}.  The production path therefore
propagates the AO density backward through the projection, SNSO map, dual
orthogonalizer, Foldy--Wouthuysen transformation, inverse square root, NESC
matrix, decoupling response, Dirac eigensystem, and canonical orthogonalizer.
...
\cref{eq:r_frechet} is self-adjoint for Hermitian perturbations, so the same
divided-difference kernel propagates the renormalization adjoint.  The
negative--positive energy denominators in \cref{eq:dirac_response} likewise
appear once in the reverse decoupling response.

The primitive adjoints are contracted directly by the integral backends.
Gaussian-nucleus $V^\xcoord$ and analytic libcint $W^\xcoord$ are generated
one atom at a time.  Consequently the production path does not allocate any
array of shape $(3N_\mathrm{atom},n,n)$ or
...
Gaussian nuclei change three pieces consistently: $V^\xcoord$,
$W^\xcoord$, and $E_\mathrm{NN}^\xcoord$.  The last is evaluated by contracting
the derivative of the Coulomb interaction between normalized Gaussian nuclear
charge functions, with diagonal self-interactions removed exactly as in the
energy.
...
\texttt{one\_electron\_deriv*} & Libint2 matrix-valued $S^\xcoord$, $T^\xcoord$, and point-$V^\xcoord$.\\
\texttt{integrals.py} & Gaussian-$V^\xcoord$, full $W^\xcoord$, component normalization, and backend selection.\\
\texttt{libcint\_two\_center.h} & Bound libcint derivative kernels.\\
\texttt{forte2/x2c/x2c.py} & X2C Hamiltonian and the matrix-valued and contracted derivative API entry points.\\
\texttt{forte2/x2c/x2c\_grad.py} & Matrix-valued forward reference, density-contracted reverse response, and direct primitive derivative contractions.\\
\bottomrule
\end{tabular}
\end{center}
...
responsible algebra:
\begin{enumerate}
\item Matrix-valued $S^\xcoord$, $T^\xcoord$, and $V^\xcoord$ are contracted
against random weights and compared with the established contracted C++ APIs.
\item All four $W^\xcoord$ components are compared with fourth-order finite
differences for point and Gaussian nuclei.  Scalar symmetry and cross-component
antisymmetry are checked explicitly.
\item $h_\mathrm{X2C}^\xcoord$ is compared directly with fourth-order finite
differences for SF-X2C, SO-X2C, both nuclear models, all SNSO variants, and a
case with a discarded overlap eigenvector.
\item The density-contracted X2C adjoint is compared with an explicit
contraction of \texttt{hcore\_deriv} against random Hermitian real and complex
densities for SF-X2C, SO-X2C, SNSO, and truncated-overlap cases.
...
Useful diagnostics are Hermiticity of $h_\mathrm{X2C}^\xcoord$, zero total
translation of an isolated molecule, and agreement of the complex GHF
collinear limit with UHF.  A small imaginary part before the final real
projection diagnoses loss of Hermiticity or a conjugation error; it should not
be discarded early to conceal such an error.
...
available, which is the default Forte2 build.  Derivative integrals using
Libint2 remain subject to the angular-momentum limit of the installed Libint2
library.  A geometry at which the number of retained overlap eigenvectors
changes is non-differentiable under hard canonical truncation and must be
handled by changing the threshold or basis.

CASSCF/GASSCF gradients are currently limited to a single optimized state,
point nuclei, and orbital spaces without frozen core, frozen virtual, active
frozen, or frozen inter-GAS rotations.  Those cases require response terms
outside the stationary single-state Lagrangian used here.
