...
restrictions.  The nonredundant orbital-pair list defines the orbital model:
it may contain the inter-GAS rotations or omit them to define a constrained
optimization with frozen inter-GAS rotations.
...
\begin{align}
 ...
 &&e\in\mathbb V.
\end{align}
The dense forward block uses exactly the matrix-free action,
\begin{equation}
 (\mathcal A^{\mathrm{oo}})_{IJ}
 =[\mathcal A^{\mathrm{oo}}\mathbf e_J]_I.
 \label{eq:nr-oo-dense}
\end{equation}
The orbital row of the Lagrange equations differentiates the scalar
$\mathbf z^{\mathsf T}\bar{\mathbf g}$ and therefore requires the adjoint
action.  Because the ordered-pair basis moves with the orbitals, define
\begin{equation}
 B[\mathbf z]
 =\dot{\bar A}[\mathbf z]+Z\bar A-\bar A Z.
 \label{eq:nr-adjoint-orbital-lagrangian}
\end{equation}
The implemented transpose Hessian--vector product is
\begin{equation}
 \boxed{
 [({\mathcal A}^{\mathrm{oo}})^{\mathsf T}\mathbf z]_I
 =2\left[B[\mathbf z]^{q_I}_{p_I}
         -B[\mathbf z]^{p_I}_{q_I}\right]
 }.
 \label{eq:nr-oo-transpose-action}
\end{equation}
The commutator is the right-trivialized moving-frame correction.  At a fully
stationary orbital solution it has no antisymmetric contribution in the
retained pair space, and the forward Hessian is symmetric.  If some rotations
are frozen, omitted components of $\bar{\mathbf g}$ may be nonzero, the
retained forward block may be nonsymmetric, and the commutator is essential.
The production response solver evaluates
\cref{eq:nr-oo-transpose-action} directly without forming
$\mathcal A^{\mathrm{oo}}$.
...
SA-CASSCF/GASSCF calculation.  With frozen inter-GAS rotations, the response
equations impose
\begin{equation}
 \kappa_f=0,
 \qquad f\notin\mathbb K,
 \label{eq:nr-frozen-orbital-constraint}
\end{equation}
and return the gradient of that constrained orbital model.  Its nuclear
connection is defined by zero antisymmetric response in the omitted
directions.  A finite-difference realization transports the reference MOs as
\begin{equation}
 C(S)=C_0\left(C_0^{\mathsf T}SC_0\right)^{-1/2}
 \label{eq:nr-symmetric-overlap-transport}
\end{equation}
before reoptimizing the retained rotations.  Restarting each displacement
from independently canonicalized parent orbitals follows a different orbital
connection and would require the corresponding parent-orbital response (for
example, CPHF); it is not the constrained derivative defined here.
...
\end{equation}
The same commutator appears in the transpose response
\cref{eq:nr-oo-transpose-action}.  Its antisymmetric contribution drops out
only at a fully stationary orbital solution and must be retained for a
constrained solution with frozen rotations.  Define
\begin{equation}
 \boxed{
...
The response solver is matrix free.  Dense orbital--orbital,
orbital--CI, CI--orbital, and CI--CI blocks are available for tests and are
defined by applying the same production matrix--vector routines to unit
vectors.  Consequently, the dense and matrix-free paths cannot drift to
different algebraic definitions.  The dense orbital--orbital matrix is the
forward derivative in \cref{eq:nr-oo-dense}; production adjoint solves apply
its transpose directly through \cref{eq:nr-oo-transpose-action}.
...
\begin{itemize}
 \item keep only $B^P_{pu}$ rather than a general--general transformed
       three-index tensor;
 \item retain only the core and active columns $\bar A^h_p$ needed by the
       moving-frame commutator rather than a full base-point Lagrangian;
