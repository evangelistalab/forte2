\documentclass[aip,preprint,nofootinbib]{revtex4-1}
...
The in-core Fock builder retains $\mathsf X$, so the metric-applied tensor
needed for derivative weights is obtained without rebuilding either integral
array:
\begin{equation}
 Z^P_{\mu\nu}
 =\sum_QN_{PQ}J^Q_{\mu\nu}
 =\sum_Q\mathsf X_{QP}B^Q_{\mu\nu}.
 \label{eq:nr-fock-reused-metric-df}
\end{equation}
The last equality follows from
$N=\mathsf X^{\mathsf T}\mathsf X$ and is valid for both the symmetric
inverse-square-root and inverse-Cholesky conventions.  The on-the-fly Fock
builder instead retains the Cholesky or truncated eigendecomposition of $M$;
it regenerates $J$ in auxiliary-shell batches but applies the cached metric
factor rather than rebuilding and refactorizing $M$.  In particular,
$B=\mathsf XJ$ cannot itself replace $Z=NJ$.
...
\end{equation}
For any orbital-dependent quantity $Q$, define its first variation along
$\mathbf z$ by
\begin{equation}
 \delta_{\mathbf z}Q
 =\left.\frac{d}{d\epsilon}
 Q\!\left(C\exp(\epsilon Z)\right)\right|_{\epsilon=0},
 \qquad
 \delta_{\mathbf z}C=CZ.
...
\begin{equation}
 ...
 =2\left[(\delta_{\mathbf z}\bar A)^{q_I}_{p_I}
         -(\delta_{\mathbf z}\bar A)^{p_I}_{q_I}\right]
 }.
...
\begin{align}
 \delta_{\mathbf z}P_{\core}
 &=(\delta_{\mathbf z}C_{\core})C_{\core}^{\mathsf T}
  +C_{\core}(\delta_{\mathbf z}C_{\core})^{\mathsf T},
 \\
 \delta_{\mathbf z}\bar P_{\act}
 &=(\delta_{\mathbf z}C_{\act})\bar\gamma C_{\act}^{\mathsf T}
  +C_{\act}\bar\gamma(\delta_{\mathbf z}C_{\act})^{\mathsf T}.
...
\begin{align}
 \delta_{\mathbf z}F_{\core}^{\mathrm{AO}}
 &=2J[\delta_{\mathbf z}P_{\core}]-K[\delta_{\mathbf z}P_{\core}],
 \\
 \delta_{\mathbf z}\bar F_{\act}^{\mathrm{AO}}
 &=J[\delta_{\mathbf z}\bar P_{\act}]
  -\frac12K[\delta_{\mathbf z}\bar P_{\act}].
 \label{eq:nr-fock-response}
\end{align}
For $X\in\{\core,\act\}$, transformation to the moving MO frame gives
\begin{equation}
 \delta_{\mathbf z}F_X=Z^{\mathsf T}F_X+F_XZ
 +C^{\mathsf T}(\delta_{\mathbf z}F_X^{\mathrm{AO}})C.
...
\begin{equation}
 (\delta_{\mathbf z}C_{\core},C_{\core},I;2,1),
 \qquad
 (\delta_{\mathbf z}C_{\act},C_{\act},\bar\gamma;1,1/2),
\end{equation}
...
\begin{align}
 ...
 &(\delta_{\mathbf z}T)^P_{tu}
 &=\sum_{vw\in\mathbb A}(\delta_{\mathbf z}B)^P_{vw}\bar D_{tu,vw},
 \label{eq:nr-df-working-rdm}
 \\
 \left.(\delta_{\mathbf z}\bar A)^{u}_{p}\right|_{2e}
 &=\sum_{Pt}\left((\delta_{\mathbf z}B)^P_{pt}T^P_{tu}
                  +B^P_{pt}(\delta_{\mathbf z}T)^P_{tu}\right).
 \label{eq:nr-df-lagrangian-response}
\end{align}
Together with the one-electron/Fock terms, the complete Lagrangian response is
\begin{align}
 (\delta_{\mathbf z}\bar A)^{m}_{p}
 &=2(\delta_{\mathbf z}F_{\core}
     +\delta_{\mathbf z}\bar F_{\act})^{m}_{p},
 &&m\in\mathbb C,
 \\
 (\delta_{\mathbf z}\bar A)^{u}_{p}
 &=\sum_{v\in\mathbb A}(\delta_{\mathbf z}F_{\core})^{v}_{p}
   \bar\gamma^{v}_{u}
  +\left.(\delta_{\mathbf z}\bar A)^{u}_{p}\right|_{2e},
 &&u\in\mathbb A,
 \label{eq:nr-lagrangian-response}
 \\
 (\delta_{\mathbf z}\bar A)^{e}_{p}&=0,
 &&e\in\mathbb V.
\end{align}
...
\begin{align}
 \delta_{\mathbf z}E_{\core}
 &=\Tr\!\left[(\delta_{\mathbf z}C_{\core})^{\mathsf T}
 (h+F_{\core}^{\mathrm{AO}})C_{\core}\right]
 +\Tr\!\left[C_{\core}^{\mathsf T}
 (h+F_{\core}^{\mathrm{AO}})(\delta_{\mathbf z}C_{\core})\right]
 \nonumber\\
 &\quad+\Tr\!\left[C_{\core}^{\mathsf T}
 (\delta_{\mathbf z}F_{\core}^{\mathrm{AO}})C_{\core}\right].
...
\begin{equation}
 \hat H[\mathbf z]
 =\delta_{\mathbf z}E_{\core}
 +\sum_{uv\in\mathbb A}(\delta_{\mathbf z}F_{\core})^u_v\hat E^u_v
 +\frac12\sum_{uvtw\in\mathbb A}
 \delta_{\mathbf z}\langle uv|tw\rangle\hat E^{uv}_{tw}.
 \label{eq:nr-response-hamiltonian}
\end{equation}
Only active four-index response integrals are formed:
\begin{align}
 (\delta_{\mathbf z}B)^P_{uv}
 &=\sum_{p\in\mathbb G}
 \left(Z_{pu}B^P_{pv}+B^P_{up}Z_{pv}\right),
 \\
 \delta_{\mathbf z}\langle uv|tw\rangle
 &=\sum_P\left((\delta_{\mathbf z}B)^P_{ut}B^P_{vw}
               +B^P_{ut}(\delta_{\mathbf z}B)^P_{vw}\right).
...
$\delta_{\mathbf z_\alpha}\bar A$, because that derivative is evaluated in a
moving MO frame.  For a general coefficient variation
$\delta_XC=CX$,
\begin{equation}
 D_X\left(\mathbf z_\alpha^{\mathsf T}\bar{\mathbf g}\right)
 =2\sum_{pq}X_{pq}
 \left[\delta_{\mathbf z_\alpha}\bar A
...
$\delta_\alpha\equiv\delta_{\mathbf z_\alpha}$; in particular,
$\delta_\alpha C=CZ_\alpha$.
...
$\delta_\alpha C_h$ the core and active columns of $CZ_\alpha$, the transformed
DF response at fixed AO $Z^P_{\mu\nu}$ is
\begin{equation}
 \delta_\alpha Z^P_{ij}
 =\sum_{\mu\nu}\left[
 (\delta_\alpha C_h)_{\mu i}Z^P_{\mu\nu}(C_h)_{\nu j}
 +(C_h)_{\mu i}Z^P_{\mu\nu}(\delta_\alpha C_h)_{\nu j}
 \right].
 \label{eq:nr-hole-df-response}
\end{equation}
Holding the barred RDMs fixed gives
\begin{align}
 \delta_\alpha\bar R^{P}
 &=\sum_{ij}\bar\gamma^h_{ij}\delta_\alpha Z^P_{ij},
 \\
 \delta_\alpha\bar X^{P}_{ij}
 &=\sum_{kl}\bar\gamma^h_{ik}\delta_\alpha Z^P_{lk}\bar\gamma^h_{lj},
 \\
 \delta_\alpha\bar L^{P}_{uw}
 &=\sum_{vt}\bar\Lambda^{\act}_{uv,wt}\delta_\alpha Z^P_{vt},
 \\
 \delta_\alpha\bar W^{P,h}_{ij}
 &=\bar\gamma^h_{ij}\delta_\alpha\bar R^{P}
 -\frac12\delta_\alpha\bar X^{P}_{ij}
...
\begin{align}
 \delta_\alpha\bar W_{PQ}
 &=-\frac12\left((\delta_\alpha\bar R^{P})\bar R^Q
                  +\bar R^P(\delta_\alpha\bar R^{Q})\right)
 \nonumber\\
 &\quad+\frac14\sum_{ij}\left(
 (\delta_\alpha Z^P_{ij})\bar X^Q_{ij}
 +Z^P_{ij}(\delta_\alpha\bar X^Q_{ij})\right)
 \nonumber\\
 &\quad-\frac12\sum_{uw}\left(
 (\delta_\alpha Z^P_{uw})\bar L^Q_{uw}
 +Z^P_{uw}(\delta_\alpha\bar L^Q_{uw})\right).
...
\begin{align}
 ...
 (\delta_\alpha C)_{\mu m}C_{\nu m}
 +C_{\mu m}(\delta_\alpha C)_{\nu m}\right)
 \nonumber\\
 &\quad+\sum_{uv\in\mathbb A}\left(
 (\delta_\alpha C)_{\mu u}\bar\gamma_{uv}C_{\nu v}
 +C_{\mu u}\bar\gamma_{uv}(\delta_\alpha C)_{\nu v}\right).
...
\begin{align}
 ...
 \nonumber\\
 &\quad+\sum_{ij}\left[
 (\delta_\alpha C_h)_{\mu i}\bar W^{P,h}_{ij}(C_h)_{\nu j}
 +(C_h)_{\mu i}\bar W^{P,h}_{ij}(\delta_\alpha C_h)_{\nu j}
 \right]
 \nonumber\\
 &\quad+\sum_{ij}(C_h)_{\mu i}
 (\delta_\alpha\bar W^{P,h}_{ij})(C_h)_{\nu j}.
 \label{eq:nr-relaxed-w3}
\end{align}
The relaxed metric weight, called \texttt{W2}, is
\begin{equation}
 W^\alpha_{PQ}=W^{\mathrm b}_{PQ}+\delta_\alpha\bar W_{PQ}.
...
\subsection{Implemented contraction sequence}

The relaxed-density implementation follows three explicit algebraic kernels.
First, the AO one-particle density is assembled directly from the core and
active blocks according to \cref{eq:nr-relaxed-ao-1rdm}.  Equivalently, if
\begin{equation}
 \mathcal D[C_{\core},C_{\act};\gamma]
 =2C_{\core}C_{\core}^{\mathsf T}
  +C_{\act}\gamma C_{\act}^{\mathsf T},
\end{equation}
then the implemented density is
\begin{equation}
 D^{\mathrm{rel},\alpha}
 =\mathcal D[C_{\core},C_{\act};\gamma^{\mathrm b}]
 +D_{\delta_\alpha C}\mathcal D[C_{\core},C_{\act};\bar\gamma],
 \label{eq:nr-implemented-relaxed-d1}
\end{equation}
where the directional derivative acts only on the coefficient matrices.

Second, the AO metric-applied tensor is transformed to the hole space together
with its orbital direction,
\begin{equation}
 (Z_h,\delta_\alpha Z_h)
 =\left(C_h^{\mathsf T}ZC_h,
 (\delta_\alpha C_h)^{\mathsf T}ZC_h
 +C_h^{\mathsf T}Z(\delta_\alpha C_h)\right),
 \label{eq:nr-implemented-hole-transform}
\end{equation}
which is \cref{eq:nr-hole-df,eq:nr-hole-df-response} written in matrix form for
each auxiliary index.  The Fock layer supplies $Z^P_{\mu\nu}$ using
\cref{eq:nr-fock-reused-metric-df}; the gradient layer does not reevaluate
$J^P_{\mu\nu}$ or $M_{PQ}$ for the in-core path.  The AO tensor
$Z^P_{\mu\nu}$ is released immediately after this transformation.
Consequently it is not resident when the final AO-sized three-center weight
$W^{P,\alpha}_{\mu\nu}$ is allocated.

Third, the base and directional compact weights are back-transformed in one
kernel according to \cref{eq:nr-relaxed-w3}.  In functional notation, with
\begin{equation}
 \mathcal B[C_h,U^h]^P=C_hU^{P,h}C_h^{\mathsf T},
\end{equation}
the implemented operation is
\begin{equation}
 W^{P,\alpha}
 =\mathcal B[C_h,W^{P,h,\mathrm b}]
 +D_{(\delta_\alpha C_h,\delta_\alpha\bar W^{P,h})}
  \mathcal B[C_h,\bar W^{P,h}].
 \label{eq:nr-implemented-w3-backtransform}
\end{equation}
The base hole density and cumulant are discarded before their state-averaged
counterparts are formed.  After the directional compact weights are built,
only the hole-space weights and coefficient blocks remain for the AO
back-transformation.  This ordering retains the
$O(N_{\mathrm{aux}}N_{\mathrm H}^2)$ compact representation until the last
step and avoids overlapping the two
$O(N_{\mathrm{aux}}N_{\mathrm{AO}}^2)$ tensors in memory.
