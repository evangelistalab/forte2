...
apply to SA-GASSCF when the nonredundant orbital-pair list contains the
appropriate inter-GAS rotations and the active-space solver imposes the GAS
...
orbitals or derivative couplings.
...
The orbital--orbital Hessian action is the directional derivative of the
state-averaged orbital gradient at fixed SA RDMs and fixed AO integrals:
\begin{equation}
 \boxed{
...
\begin{align}
 ...
 &&e\in\mathbb V.
\end{align}
The dense block uses exactly the matrix-free action,
\begin{equation}
...
state solver.  They need not all have the same dimension.
...
different algebraic definitions.
