 ...
 */
void CIVector::Hamiltonian(CIVector& result, std::shared_ptr<ActiveSpaceIntegrals> fci_ints) {
    //    check_temp_space();
    result.zero();
...
void CIVector::H0(CIVector& result, std::shared_ptr<ActiveSpaceIntegrals> fci_ints) {
...
void CIVector::H1(CIVector& result, std::shared_ptr<ActiveSpaceIntegrals> fci_ints, bool alfa) {
...
void CIVector::H2_aaaa2(CIVector& result, std::shared_ptr<ActiveSpaceIntegrals> fci_ints,
                        bool alfa) {
    for (const auto& [nI, class_Ia, class_Ib] : lists_->determinant_classes()) {
        if (lists_->detpblk(nI) == 0)
            continue;
...
void CIVector::H2_aabb(CIVector& result, std::shared_ptr<ActiveSpaceIntegrals> fci_ints) {
