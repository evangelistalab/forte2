...
class StringClass {
  public:
    ...
    size_t alpha_string_class(const String& s) const;
  ...
  private:
    ...
    std::map<std::array<int, 6>, size_t> alpha_occupation_group_;
    ...
    std::vector<std::pair<size_t, size_t>> alpha_string_classes_;
    ...
    /// (alpha class index, beta class index)
    std::map<std::pair<size_t, size_t>, size_t> alpha_string_classes_map_;
    /// A list of the classes of the beta strings stored as tuple
    /// (occupation group idx, symmetry)
    std::map<std::pair<size_t, size_t>, size_t> beta_string_classes_map_;
    /// A list of the product of alpha and beta string classes stored as tuple
    /// (product class index, alpha class index, beta class index)
