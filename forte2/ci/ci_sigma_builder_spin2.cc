...
double CISigmaBuilder::compute_spin2() {
    ...
    for (const auto& [nI, class_Ia, class_Ib] : lists_->determinant_classes()) {
        size_t block_size = alpha_address_->strpcls(class_Ia) * beta_address_->strpcls(class_Ib);
        if (block_size == 0)
            continue;
        ...
        for (const auto& [nJ, class_Ja, class_Jb] : lists_->determinant_classes()) {
            ...
            const auto& pq_vo_alpha = lists_->get_alpha_vo_list(class_Ia, class_Ja);
            const auto& pq_vo_beta = lists_->get_beta_vo_list(class_Ib, class_Jb);
            // loop over the alpha (p,q) pairs
            for (const auto& [pq, vo_alpha_list] : pq_vo_alpha) {
                const auto& [p, q] = pq;
                ...
                for (const auto& [sign_a, Ia, Ja] : vo_alpha_list) {
    ...
    double na = alpha_address_->nones();
