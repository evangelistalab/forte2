 ...
 */
std::shared_ptr<RDMs> CIVector::compute_rdms(CIVector& C_left, CIVector& C_right, int max_rdm_level,
                                             RDMsType type) {
 ...
 */
ambit::Tensor CIVector::compute_2rdm_aa_same_irrep(CIVector& C_left, CIVector& C_right, bool alfa) {
    size_t ncmo = C_left.ncmo_;
    const auto& alfa_address = C_left.alfa_address_;
    const auto& beta_address = C_left.beta_address_;
    const auto& lists = C_left.lists_;
...
ambit::Tensor CIVector::compute_3rdm_aaa_same_irrep(CIVector& C_left, CIVector& C_right,
                                                    bool alfa) {
    size_t ncmo = C_left.ncmo_;
    const auto& alfa_address = C_left.alfa_address_;
    const auto& beta_address = C_left.beta_address_;
...
ambit::Tensor CIVector::compute_3rdm_aab_same_irrep(CIVector& C_left, CIVector& C_right) {
...
ambit::Tensor CIVector::compute_3rdm_abb_same_irrep(CIVector& C_left, CIVector& C_right) {
    // this threshold is used to avoid computing very small contributions to the 3-RDM
    // here we screen for the absolute value of the elements of the C vector, and since the vector
    // is normalized, we can guarantee that the neglected contribution to the 3-RDM is smaller or
    // equal than this value
...
void CIVector::test_rdms(CIVector& Cl, CIVector& Cr, int max_rdm_level, RDMsType type,
