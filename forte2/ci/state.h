...
class State : public VectorSpace<State, Determinant, sparse_scalar_t, Determinant::Hash> {
  public:
    using base_t = VectorSpace<State, Determinant, sparse_scalar_t, Determinant::Hash>;
...
// State apply_operator_lin(const SparseOperator& op, const State& state,
...
// /// @param screen_thresh the threshold to screen the operator
// /// @return the new state
// State apply_operator_antiherm(const SparseOperator& op, const State& state,
//                                     double screen_thresh = 1.0e-12);

// /// compute the projection  <state0 | op | ref>, for each operator op in gop
// std::vector<sparse_scalar_t> get_projection(const SparseOperatorList& sop, const State&
// ref,
//                                             const State& state0);

// /// apply the number projection operator P^alpha_na P^beta_nb |state>
// State apply_number_projector(int na, int nb, const State& state);

// /// compute the overlap value <left_state|right_state>
// sparse_scalar_t overlap(const State& left_state, const State& right_state);

// /// compute the S^2 expectation value
// sparse_scalar_t spin2(const State& left_state, const State& right_state);
