...
class AVAS(MOsMixin, SystemMixin, MOSpaceMixin):
    ...
    def _make_avas_orbitals(self):
        ...
        logger.log_info1(
            f"\nAVAS: Performing energy weighting of the projected overlap matrix with parameter {self.energy_weighting}"
        )
        eps = self.parent_method.eps[0]
        e_homo = eps[ndocc - 1]
        e_lumo = eps[ndocc + nsocc]
        logger.log_info1(f"HOMO energy: {e_homo}")
        logger.log_info1(f"LUMO energy: {e_lumo}")
        exp_i = np.exp(-self.energy_weighting * np.abs(eps[:ndocc] - e_homo))
        exp_a = np.exp(-self.energy_weighting * np.abs(eps[ndocc + nsocc :] - e_lumo))
        if nsocc > 0:
            explist = np.concatenate([exp_i, np.ones(nsocc), exp_a])
        else:
            explist = np.concatenate([exp_i, exp_a])
        CpsC = np.einsum("i,ij,j->ij", explist, CpsC, explist, optimize=True)
