...
std::vector<double> SelectedCIHelper::compute_spin2() {
    local_timer spin2_timer;

    const double sz = 0.5 * (na_ - nb_);
    const auto s2_sz_contribution = sz * (sz + 1.0);
    std::vector<double> s2_expectation_values(nroots_, s2_sz_contribution);

    for (size_t b{0}; b < num_batches_per_thread_; ++b) {
        auto s2_expectation_values_batch = spin2_batch(num_batches_per_thread_, b);
        for (size_t r{0}; r < nroots_; ++r) {
            s2_expectation_values[r] += s2_expectation_values_batch[r];
        }
    }
    spin2_time_ = spin2_timer.elapsed_seconds();
    return s2_expectation_values;
}

std::vector<double> SelectedCIHelper::spin2_batch(size_t num_batches, size_t batch_id) {
    ...
    for (size_t i{0}; i < a_string_size; ++i) {
        ...
        const double a_str_phase = ((a_str.count() & 1) == 0) ? 1.0 : -1.0;
        ...
        for (const auto& a : avir) {
            ...
            double a_sign = new_a_str.create(a);
            ...
            for (const auto& [b_str_idx, det_index] : second_string_to_det_index) {
                ...
                if (b_str.get_bit(a)) {
                    double total_sign = a_sign;
                    String new_b_str = b_str;
                    total_sign *= new_b_str.destroy(a);
                    new_det.set_b_string(new_b_str);
                    for (size_t r{0}; r < nroots_; ++r) {
                        s_plus_map[r][new_det] += total_sign * c_[det_index * nroots_ + r];
                    }
                }
            }
        }
