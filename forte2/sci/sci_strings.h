...
/// @tparam StringT String: each determinant splits into an alpha (first) and beta (second)
/// string. SpinorString: the whole determinant is the first string and the second is empty.
template <typename StringT> class SelectedCIStringsImpl {
  public:
    // == Class Constructors ==
    /// @brief Construct from a list of determinants
    SelectedCIStringsImpl(size_t norb, std::vector<Determinant>& sorted_dets);
...
using SelectedCIStrings = SelectedCIStringsImpl<String>;
using RelSelectedCIStrings = SelectedCIStringsImpl<SpinorString>;
