...
void SelectedCIHelper::select_hbci_batch(DetRootMap& V_map, DetRootMap& PT_map,
    ...
    auto accumulate = [&](DetRootMap& map, std::vector<double>& coeffs, const Determinant& det,
                          double prefactor, std::span<const double> c) {
    ...
    generate_connections(
        std::span<const double>(c_), pt2_threshold, num_batches, batch_id, existing_dets,
        [&](const Determinant& det, std::span<const double> c_parent, size_t /*det_index*/,
            double coupling, double criterion) {
            if (criterion > var_threshold) {
                accumulate(V_map, V_coeffs, det, coupling, c_parent);
            } else {
                accumulate(PT_map, PT_coeffs, det, coupling, c_parent);
            }
        });
}
