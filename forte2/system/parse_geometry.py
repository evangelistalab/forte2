...
class _GeometryHelper:
    ...
    def __post_init__(self):
        ...
        sorted_axis = sorted(
            zip(axis_order, moi, range(3)), key=lambda x: (x[0], -x[1]) # sort axes by Cn order first, then by descending MOI
        )
        logger.log_debug('Sorted Axis Order:')
        for ax in sorted_axis:
            n, I, idx = ax
            logger.log_debug(f'Axis: {moi_vectors[:, idx]}   Cn: {n}   MOI: {I}   Axis Assignment: {idx}')

        self.prinrot = np.concatenate([moi_vectors[:, n][:, np.newaxis] for _, _, n in sorted_axis], axis=1).T # for rotated water
        ...
        logger.log_info1('Original atomic coordinates overwritten with principle atomic positions')
