...
SparseOperator sparse_operator_hamiltonian(double scalar_energy,
    ...
    if (one_electron_integrals.shape(1) != nspinor || two_electron_integrals.shape(0) != nspinor ||
        two_electron_integrals.shape(1) != nspinor || two_electron_integrals.shape(2) != nspinor ||
        two_electron_integrals.shape(3) != nspinor) {
        throw std::runtime_error("One-electron and two-electron integrals must be square matrices "
                                 "of the same size.");
    }
    SparseOperator H;
    ...
    for (size_t p = 0; p < nspinor; p++) {
        for (size_t q = p + 1; q < nspinor; q++) {
            for (size_t r = 0; r < nspinor; r++) {
