...
template <bool IsBeta, bool Spinor> inline double destroy_spin(Determinant& J, std::size_t p) {
    if constexpr (Spinor) {
        constexpr std::size_t n = Determinant::norb();
        return p < n ? J.destroy_alpha(p) : J.destroy_beta(p - n);
    } else if constexpr (IsBeta)
        return J.destroy_beta(p);
    else
        return J.destroy_alpha(p);
}

template <bool IsBeta, bool Spinor> inline double create_spin(Determinant& J, std::size_t p) {
    if constexpr (Spinor) {
        constexpr std::size_t n = Determinant::norb();
        return p < n ? J.create_alpha(p) : J.create_beta(p - n);
    } else if constexpr (IsBeta)
...
template <int N, std::uint32_t SpinMask, bool Spinor, std::size_t... K>
inline double apply_excitation(Determinant& J, const std::array<std::size_t, 2 * N>& idx,
    ...
    ((sign *= destroy_spin<((SpinMask >> K) & 1u) != 0, Spinor>(J, idx[rank + K])), ...);
    ((sign *= create_spin<((SpinMask >> (rank - 1 - K)) & 1u) != 0, Spinor>(J, idx[rank - 1 - K])),
     ...);
    return sign;
}
...
/// @tparam Spinor If true, index p is bit p of the determinant and SpinMask is ignored
...
template <int N, std::uint32_t SpinMask, Accum A, bool Spinor = false>
auto compute_nrdm(const SparseState& state_left, const SparseState& state_right, std::size_t norb) {
    ...
    for (std::size_t flat{0}; flat < size; ++flat) {
        ...
        for (const auto& [I, c_I] : state_right) {
            J = I;
            const double sign = detail::apply_excitation<N, SpinMask, Spinor>(
                J, idx, std::make_index_sequence<N>{});
            if (sign != 0) {
...
// == Two-component RDMs ==
//
// Spinor p is bit p of the determinant, so spinors 64 and above occupy the beta word. These
...
inline auto compute_1rdm_2c(const SparseState& state_left, const SparseState& state_right,
                            std::size_t norb) {
    return compute_nrdm<1, spin_mask("a"), Accum::Hermitian, true>(state_left, state_right, norb);
}

inline auto compute_2rdm_2c(const SparseState& state_left, const SparseState& state_right,
                            std::size_t norb) {
    return compute_nrdm<2, spin_mask("aa"), Accum::Hermitian, true>(state_left, state_right, norb);
}

inline auto compute_3rdm_2c(const SparseState& state_left, const SparseState& state_right,
                            std::size_t norb) {
    return compute_nrdm<3, spin_mask("aaa"), Accum::Hermitian, true>(state_left, state_right, norb);
}
