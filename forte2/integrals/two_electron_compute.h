...
// Compute the shell-pair Schwarz factors Q_AB = sqrt(max_{a in A, b in B} (ab|ab)) for every
// ordered shell pair (A, B), laid out row-major over the shell-pair index A * nshells + B (length
// nshells * nshells). By the Cauchy-Schwarz inequality |(ab|cd)| <= sqrt((ab|ab)) sqrt((cd|cd)) <=
// Q_AB Q_CD, so a shell-quartet whose Q_AB * Q_CD is below the decomposition threshold contributes
// nothing and need never be evaluated. This is the screening input consumed by
// compute_two_electron_4c_pair_block_screened.
template <libint2::Operator Op, typename Params = NoParams>
[[nodiscard]] auto compute_two_electron_4c_schwarz_factors(const Basis& basis,
    ...
    auto q = make_zeros<nb::numpy, double, 1>({nshells * nshells});
    auto data = q.data();

    // Parallelize over the first shell of the pair; each thread owns its own engine (libint2
    // engines are not thread-safe) and writes disjoint rows of the shell-pair table.
    auto kernel = [&](std::size_t s1_begin, std::size_t s1_end) {
        libint2::Engine engine(Op, max_nprim, max_l);
        if constexpr (!std::is_same_v<Params, NoParams>) {
            engine.set_params(params);
        ...
        for (std::size_t s1 = s1_begin; s1 < s1_end; ++s1) {
            ...
            for (std::size_t s2 = 0; s2 < nshells; ++s2) {
                ...
                // Compute the shell quartet (s1 s2 | s1 s2) and reduce its diagonal (ij|ij) to the
                // per-shell-pair maximum.
                engine.compute(shell1, shell2, shell1, shell2);
                const auto buf = results[0];
                if (!buf)
                    continue;
                ...
                for (std::size_t i = 0; i < n1; ++i) {
                    for (std::size_t j = 0; j < n2; ++j) {
                        const std::size_t ijij = ((i * n2 + j) * n1 + i) * n2 + j;
                        const double v = static_cast<double>(buf[ijij]);
                        if (v > qmax)
                            qmax = v;
                    }
                }
                data[s1 * nshells + s2] = std::sqrt(qmax);
            }
        }
    };
    ...
    const auto end = std::chrono::high_resolution_clock::now();
    const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    LOG_INFO2 << "[forte2] Four-center Schwarz-factor timing: " << elapsed.count() / 1000.0
              << " s\n";

    return q;
}

// Schwarz-screened variant of compute_two_electron_4c_pair_block. The output layout and contract
// are identical, but a shell-quartet (AB|CD) whose Cauchy-Schwarz bound Q_AB * Q_CD < tau is
// skipped entirely -- libint2 is never asked to evaluate it and the corresponding block stays zero.
// The screening only removes contributions provably below tau, so the result equals the unscreened
// block to within tau. schwarz is the (nshells * nshells) table from
// compute_two_electron_4c_schwarz_factors, indexed A * nshells + B.
template <libint2::Operator Op, typename Params = NoParams>
[[nodiscard]] auto
compute_two_electron_4c_pair_block_screened(const Basis& basis, const np_matrix_int& bra_pairs,
                                            const np_matrix_int& ket_pairs,
    ...
    if (schwarz.shape(0) != nshells * nshells) {
        throw std::invalid_argument("schwarz must have length nshells * nshells");
    }

    auto bra_view = bra_pairs.view();
    auto ket_view = ket_pairs.view();
    const auto qdata = schwarz.data();
    ...
    auto kernel = [&](std::size_t pb_begin, std::size_t pb_end) {
        ...
        for (std::size_t pb = pb_begin; pb < pb_end; ++pb) {
            ...
            const std::size_t nB = first_size[B].second;
            const std::size_t r0 = row_off[pb];
            const double q_ab = qdata[A * nshells + B];

            for (std::size_t pk = 0; pk < n_ket; ++pk) {
                const auto [C, D] = ket_sh[pk];
                const std::size_t c0 = col_off[pk];

                // Cauchy-Schwarz prescreen: skip the quartet (and leave its block zero) when its
                // bound is below the threshold, never calling the engine for negligible quartets.
                if (q_ab * qdata[C * nshells + D] < tau)
                    continue;
                ...
                const auto buf = results[0];
                if (!buf)
                    continue; // engine-screened quartet: block stays zero
    ...
    const auto end = std::chrono::high_resolution_clock::now();
    const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    LOG_INFO2 << "[forte2] Four-center screened pair-block timing: " << elapsed.count() / 1000.0
              << " s\n";
