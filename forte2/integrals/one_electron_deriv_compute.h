                    ...
                    if (buf_x1 || buf_y1 || buf_z1 || buf_x2 || buf_y2 || buf_z2) {
                        for (std::size_t i = 0, ij = 0; i != n1; ++i) {
                            for (std::size_t j = 0; j != n2; ++j, ++ij) {
                                ...
                                grad_view(a1 * 3 + 0) += static_cast<double>(buf_x1[ij]) * dm_val;
                                grad_view(a1 * 3 + 1) += static_cast<double>(buf_y1[ij]) * dm_val;
                                grad_view(a1 * 3 + 2) += static_cast<double>(buf_z1[ij]) * dm_val;
                                grad_view(a2 * 3 + 0) += static_cast<double>(buf_x2[ij]) * dm_val;
                                grad_view(a2 * 3 + 1) += static_cast<double>(buf_y2[ij]) * dm_val;
                                grad_view(a2 * 3 + 2) += static_cast<double>(buf_z2[ij]) * dm_val;
                            }
                        }
                    }
                }
