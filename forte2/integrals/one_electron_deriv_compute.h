...
[[nodiscard]] auto compute_one_electron_deriv_matrices(
    const Basis& basis1, const Basis& basis2,
    const std::vector<std::pair<double, std::array<double, 3>>>& charges) -> np_tensor3 {
    const std::size_t nb1 = basis1.size();
    const std::size_t nb2 = basis2.size();

    validate_deriv_centers(basis1, charges, "basis1", "compute_one_electron_deriv_matrices");
    validate_deriv_centers(basis2, charges, "basis2", "compute_one_electron_deriv_matrices");
    ...
    const auto center_to_shell1 = basis1.center_first_and_last(true);
    const auto center_to_shell2 = basis2.center_first_and_last(true);

    const std::size_t natoms = charges.size();
    const std::size_t n_deriv_components = 6 + (op_is_pos_dep ? 3 * natoms : 0);
    auto deriv = make_zeros<nb::numpy, double, 3>({3 * natoms, nb1, nb2});
    auto deriv_view = deriv.view();

    for (std::size_t atom1 = 0; atom1 < natoms; ++atom1) {
        const auto [first_shell1, last_shell1] = center_to_shell1[atom1];
        for (std::size_t atom2 = 0; atom2 < natoms; ++atom2) {
            const auto [first_shell2, last_shell2] = center_to_shell2[atom2];
            for (std::size_t shell1 = first_shell1; shell1 < last_shell1; ++shell1) {
                const auto [first1, size1] = first_size1[shell1];
                for (std::size_t shell2 = first_shell2; shell2 < last_shell2; ++shell2) {
                    const auto [first2, size2] = first_size2[shell2];
                    engine.compute(basis1[shell1], basis2[shell2]);

                    for (std::size_t component = 0; component < n_deriv_components; ++component) {
                        const auto buffer = results[component];
                        if (!buffer) {
                            continue;
                        }
                        ...
                        for (std::size_t i = 0, ij = 0; i < size1; ++i) {
                            for (std::size_t j = 0; j < size2; ++j, ++ij) {
                                deriv_view(3 * atom + cart, first1 + i, first2 + j) +=
                                    static_cast<double>(buffer[ij]);
                            }
                        }
                    }
                }
