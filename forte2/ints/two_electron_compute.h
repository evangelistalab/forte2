            ...
            for (std::size_t s3 = 0; s3 < nshells3; ++s3) {
                ...
                for (std::size_t s4 = 0; s4 < nshells4; ++s4) {
                    ...
                    if (buf) {
                        ...
                        const std::size_t n34 = n3 * n4;
                        const std::size_t n234 = n2 * n34;
                        size_t idx = 0;
...
[[nodiscard]] auto compute_two_electron_3c_multi(const Basis& basis1, const Basis& basis2,
    ...
    const std::size_t nb2 = basis2.size();
    const std::size_t nb3 = basis3.size();
    const std::size_t nb123 = nb1 * nb2 * nb3;
    const std::size_t nb23 = nb2 * nb3;
    ...
    // Allocate a flat buffer
    auto buffer = std::make_unique<std::vector<double>>(nb123, 0.0);
    auto& data = *buffer;
                ...
                if (buf) {
                    ...
                    for (std::size_t i = 0; i < n1; ++i) {
                        for (std::size_t j = 0; j < n2; ++j) {
                            ...
                            const auto off_buf_12 = i * n23 + j * n3;
                            for (std::size_t k = 0; k < n3; ++k) {
                                data[off12 + k] = static_cast<double>(buf[off_buf_12 + k]);
                            }
                        }
                    }
                }
    ...
    return make_ndarray<nb::numpy, double, 3>(std::move(buffer),
                                              std::array<std::size_t, 3>{nb1, nb2, nb3});
}
...
[[nodiscard]] auto compute_two_electron_2c_multi(const Basis& basis1, const Basis& basis2,
    ...
    const auto nb12 = nb1 * nb2;

    // Allocate a flat buffer
    auto buffer = std::make_unique<std::vector<double>>(nb12, 0.0);
    auto& data = *buffer;

    // Loop over the shells in basis1
    for (std::size_t s1 = 0; s1 < nshells1; ++s1) {
        const auto& shell1 = basis1[s1];
