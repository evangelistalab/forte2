...
class LBFGS:
    ...
    def minimize(self, obj, x):
        ...
        if not self.converged and self.warn_if_not_converged:
            logger.log_warning(
                f"  L-BFGS Warning: No convergence in {self.iter} iterations"
            )
    ...
    def _update(self):
        ...
        for k in range(m):
            i = (end - k + m) % m
            self.alpha[i] = self.rho[i] * np.dot(self.s[i].conj(), self.p)
            self.p -= self.alpha[i] * self.y[i]

        self.p = self._apply_h0(self.p)

        for k in range(m):
            i = (end + k + 1) % m
            beta = self.rho[i] * np.dot(self.y[i].conj(), self.p)
    ...
    def _scale_direction_vector(self, func, x, fx, step):
        ...
        fx = func.evaluate(x)
        if do_grad:
            self.g = func.gradient(x)
        return fx, step

    def _line_search_backtracking(self, func, x, fx, step):
        dg0 = np.dot(self.g.conj(), self.p)
        fx0 = fx
        x0 = x.copy()
    ...
    def _resize(self, m):
        ...
        self.alpha = np.zeros(m, dtype=self.dtype)
        self.rho = np.zeros(m, dtype=self.dtype)
