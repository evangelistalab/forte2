...
def getints(
    ...
    if any(bas[:, ANG_OF] > 12):
        raise NotImplementedError(
            "cint library does not support high angular (l>12) GTOs"
        )

    if (
        intor_name.startswith("int1e")
        or intor_name.startswith("ECP")
        or intor_name.startswith("int2c2e")
    ):
        return getints2c(
            intor_name, atm, bas, env, shls_slice, comp, hermi, ao_loc, cintopt, out
        )
    elif intor_name.startswith("int2e") or intor_name.startswith("int4c1e"):
        return getints4c(
            intor_name, atm, bas, env, shls_slice, comp, aosym, ao_loc, cintopt, out
        )
...
def getints2c(
    ...
    if intor_name.endswith("_cart") or intor_name.endswith("_sph"):
        ...
        if "2c2e" in intor_name:
            assert hermi != HERMITIAN and hermi != ANTIHERMI
...
def getints3c(
    ...
    if shls_slice is None:
        ...
        assert shls_slice[1] <= nbas and shls_slice[3] <= nbas and shls_slice[5] <= nbas
...
def getints4c(
    ...
    if aosym == "s8":
        ...
        if shls_slice is None:
            ...
            assert (
                shls_slice[1] <= nbas
                and shls_slice[3] <= nbas
                and shls_slice[5] <= nbas
                and shls_slice[7] <= nbas
            )
        ...
        if "_spinor" in intor_name:
            ...
            out = numpy.ndarray(
                shape[::-1], dtype=numpy.complex128, buffer=out, order="F"
            )
...
def getints_by_shell(intor_name, shls, atm, bas, env, comp=1):
    ...
    elif intor_name.startswith("int2c") or "1e" in intor_name or "ECP" in intor_name:
        assert len(shls) == 2
        di = num_cgto_of(shls[0])
        dj = num_cgto_of(shls[1])
...
def make_cintopt(atm, bas, env, intor):
    ...
    natm = c_atm.shape[0]
    nbas = c_bas.shape[0]
    cintopt = ctypes.cast(c_null_ptr(), _cintoptHandler)
    cintopt.intor = intor
...
def get_integral(system, intor_name, nucmod=None):
    atom = system.atoms
    basis = atom_basis_to_bas(system.atom_basis)
    env = numpy.zeros(PTR_ENV_START)
    atm, bas, env = make_env(atom, basis, env, nucmod)
    intor, comp = _get_intor_and_comp(intor_name)
    return getints(intor, atm, bas, env, comp=comp)
