...
def cas_energy_given_cumulants(E_core, H_cas, V_cas, gamma1, lambda2):
    # see eq B.3. of JCP 146, 124132 (2017), but instead of gamma2, use lambda2
    e1 = np.einsum("uv,uv->", H_cas, gamma1, optimize=True)
    e2 = 0.5 * np.einsum("uvxy,uvxy->", V_cas, lambda2, optimize=True)
    e2 += 0.5 * np.einsum("uvxy,ux,vy->", V_cas, gamma1, gamma1, optimize=True)
    e2 -= 0.5 * np.einsum("uvxy,uy,vx->", V_cas, gamma1, gamma1, optimize=True)
    return E_core + e1 + e2