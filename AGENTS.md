...
the orbitals.
 ...
 skeletons, sub-solver fan-out, state-averaged RDMs and cumulants, and transition properties.
 Concrete solvers supply two class-level hooks:
...
The `final_orbitals` option (`"original"` / `"semicanonical"` / `"natural"` / `"ibo"` /
`"ibo_atomic"`) is a driver option, declared per driver because the default legitimately differs
(`"original"` for `CI`, `"semicanonical"` for `MCOptimizer`) and validated through
`forte2/orbitals/final_orbitals.py`.
